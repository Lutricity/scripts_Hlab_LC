...
def save_all_chip_piece_variant_for_motif(project_name, motif_name, df, df_result, motif_seq_1, motif_seq_2, motif_ss_1, motif_ss_2):
    ...
    for i, row in df.iterrows():
        motif_tile_output = tile_motif_in_chip_sequence(row.sequence, row.ss, motif_seq_1, motif_seq_2, motif_ss_1, motif_ss_2)
        for l in motif_tile_output:
            #if l[1] ==
#input chip_pieces dataframe needs a column named 'helix' (index) and a column named 'name'
            df_result.loc[pos] = [project_name, motif_name, row['helix'], row['name'], l[0], l[1], motif_seq_1 + "+" + motif_seq_2,
...
def main():
    ...
    df = pd.read_csv("chip_pieces/test_8bp_all_4LVV_chip_pieces_copy.csv")
    #print("This is chip pieces", df.head())
    #print(df.dtypes)
    #print(type(df['name'].iloc[1])

    #Made blank pandas df to begin filling resulting chip pieces into
    df_result = pd.DataFrame(columns='project_name motif_name chip_type chip_sequence chip_structure insert_sequence insert_structure helix'.split())
...
#Below this comment begins trying to match chip_structure against RNAfold's predicted secondary structure for a sequence.
    ...
    final_df.to_csv("8bp_1A_4LVV.4_4LVV_RNAfolded_20200915.csv", index=False)
