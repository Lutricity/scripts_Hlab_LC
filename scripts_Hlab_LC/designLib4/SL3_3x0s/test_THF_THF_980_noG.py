...
def tile_motif_in_chip_sequence(chip_seq, chip_ss, seq_1, seq_2, ss_1, ss_2):
    ...
    length_offset = 0 #(len(chip_seq) - 57) // 2
    ...
    #print(helix_5prime)
    new_helix_5prime = seq_1 + helix_5prime[motif_length:start_hair]
    new_helix_3prime = helix_3prime[0:-motif_length] + seq_2

    ss_new_helix_5prime = ss_1 + ss_helix_5prime[motif_length:start_hair ]
...
def main():
    ...
    df = pd.read_csv("980_noG_chip_pieces.csv")
...
#Below this comment begins trying to match chip_structure against RNAfold's predicted secondary structure for a sequence.
    ...
    final_df.to_csv("3A_noG_980_980.9_RNAfolded_20200920.csv", index=False)
