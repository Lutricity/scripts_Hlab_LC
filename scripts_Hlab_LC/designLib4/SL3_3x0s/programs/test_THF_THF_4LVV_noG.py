...
def tile_motif_in_chip_sequence(chip_seq, chip_ss, seq_1, seq_2, ss_1, ss_2):
    ...
    length_offset = 0#(len(chip_seq) - 55) // 2
...
def main():
    ...
    df = pd.read_csv("4LVV_chip_pieces_noG.csv")
...
#Below this comment begins trying to match chip_structure against RNAfold's predicted secondary structure for a sequence.
    ...
    for i, row in final_df.iterrows():
        ...
        if final_df['chip_structure'][i][11:] != final_df['RNAfold_structure'][i][11:] and final_df['chip_structure'][i][20:28] != final_df['RNAfold_structure'][i][20:28] and final_df['chip_structure'][i][:-15] != final_df['RNAfold_structure'][i][:-15]:
    ...
    final_df.to_csv("3B_all_4LVV.4_4LVV_RNAfolded_noG_20200920.csv", index=False)
