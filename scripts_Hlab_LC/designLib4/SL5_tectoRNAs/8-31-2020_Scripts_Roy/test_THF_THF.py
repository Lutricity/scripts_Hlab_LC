...
def tile_motif_in_chip_sequence(chip_seq, chip_ss, seq_1, seq_2, ss_1, ss_2):
    ...
    start_hair = 21 + length_offset
    end_hair = 25 + length_offset
...
def main():
    ...
    df_motifs = pd.read_csv("motifs_20200721_sublib_1B.csv")

    #Load in the empty chip piece to tile motifs into
    df = pd.read_csv("/Users/Phantom/projects/tecto_rna_design/LC_designed_TAR_Lib_20200403/chip_pieces/8bp_all_4LVV_chip_pieces.csv")
    ...
    ###### DEBUG #######
    print("Checkpoint: 1 Load input dfs --- %s seconds ---" % (time.time() - start_time))
    start_time = time.time()
    ###### DEBUG #######
...
# finally, save_all_chip_piece_variant_for_motif is called again n the flipped motif
    ...
    ###### DEBUG #######
    print("Checkpoint: 2: define inputs --- %s seconds ---" % (time.time() - start_time))
    start_time = time.time()
    ###### DEBUG #######
    ...
    ###### DEBUG #######
    print("Checkpoint: 3: make seqs/ss --- %s seconds ---" % (time.time() - start_time))
    start_time = time.time()
    ###### DEBUG #######
    ...
    ###### DEBUG #######
    print("Checkpoint: 4: make flipped seqs/ss --- %s seconds ---" % (time.time() - start_time))
    start_time = time.time()
    ###### DEBUG #######
    ...
    ###### DEBUG #######
    print("Checkpoint: 5: save flipped seq/ss --- %s seconds ---" % (time.time() - start_time))
    start_time = time.time()
    ###### DEBUG #######
...
#Below this comment begins trying to match chip_structure against RNAfold's predicted secondary structure for a sequence.
    ...
    ###### DEBUG #######
    print("Checkpoint: 6: make seq df to RNAfold --- %s seconds ---" % (time.time() - start_time))
    start_time = time.time()
    ###### DEBUG #######
    ...
    ###### DEBUG #######
    print("Checkpoint: 7: RNAfold seqs --- %s seconds ---" % (time.time() - start_time))
    start_time = time.time()
    ###### DEBUG #######
    ...
    final_df.to_csv("seqs_RNAfolded_TAR_1C_sameLen_20200721.csv", index=False)

    ###### DEBUG #######
    print("Checkpoint: 8: compare ss to RNAfolded & save --- %s seconds ---" % (time.time() - start_time))
    ###### DEBUG #######
