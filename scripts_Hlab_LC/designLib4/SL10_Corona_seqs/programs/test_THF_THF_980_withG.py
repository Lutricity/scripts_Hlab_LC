...
def main():
    ...
    df_motifs = pd.read_csv("../motifs/SARS-CoV-2_RNA-MaP_Structures.csv")

    #Load in the empty chip piece to tile motifs into
    df = pd.read_csv("980_withG_chip_pieces_4.csv")
...
#Below this comment begins trying to match chip_structure against RNAfold's predicted secondary structure for a sequence.
    ...
    for i, row in final_df.iterrows():
        ...
        if final_df['chip_structure'][i][:12] != final_df['RNAfold_structure'][i][:12]:
            final_df.drop(i, inplace=True)
        else:
            pass
    for i, row in final_df.iterrows():
        if final_df['chip_structure'][i][-13:] != final_df['RNAfold_structure'][i][-13:]:
            final_df.drop(i, inplace=True)
        else:
            pass
    #for i, row in final_df.iterrows():
     #   if ")" in final_df['RNAfold_structure'][i][:20]:
      #      final_df.drop(i, inplace=True)
       # else:
        #    pass
    print(final_df.head())
    ...
    final_df.to_csv("10_corona_980_980.9_RNAfolded_20200924.csv", index=False)
