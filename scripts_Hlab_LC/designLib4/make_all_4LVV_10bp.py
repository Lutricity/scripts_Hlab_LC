...
import pandas as pd
from support_THF_4LVV_10bp import *
import subprocess
import numpy as np
...
def main():
    ...
    df_motifs = pd.read_csv("input_motifs/TAR_20200821_1A_10bp.csv")


    # Load in the empty chip piece to tile motifs into
    for i in range(9831):
        count = 654 + i
        name = "chip_pieces/10bp_helices/10bp_4LVV_Hs" + str(count) + ".csv"
...
# finally, save_all_chip_piece_variant_for_motif is called again n the flipped motif
        ...
        df_result.to_csv("20200915_seqs_pre_RNAfolded.csv", index=False)
    ...
    #Below this comment begins trying to match chip_structure against RNAfold's predicted secondary structure for a sequence.
        ...
        subprocess.call("RNAfold -T20 --noLP chip_seqs_to_RNAfold.csv > out_10bp", shell = True)
        ...
        for j, row in final_df.iterrows():
            ...
            if final_df['chip_structure'][j][10:] != final_df['RNAfold_structure'][j][10:] and final_df['chip_structure'][j][:-15] !=  final_df['RNAfold_structure'][j][:-15]:
                final_df.drop(j, inplace=True)

        #print(final_df.head())
        #print("After cutting misfolded constructs", final_df.shape)

        new_name = "folded/10bp/10bp_4LVV.4_4LVV_RNAfolded_20200915"+ num + ".csv"
