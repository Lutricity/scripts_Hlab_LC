...
def main():
    ...
    df_motifs = pd.read_csv("../motifs/core_kt_alt_mismatches.csv")
    print(df_motifs)
    #Load in the empty chip piece to tile motifs into
    df = pd.read_csv("4_11ntR_chip_pieces.csv")
...
#Below this comment begins trying to match chip_structure against RNAfold's predicted secondary structure for a sequence.
    ...
    for i, row in final_df.iterrows():
        if final_df['chip_structure'][i][:11] != final_df['RNAfold_structure'][i][:11] and final_df['chip_structure'][i][7:] != final_df['RNAfold_structure'][i][7:]:
            #final_df["motif_name"][i] = 'misfold_' + final_df["motif_name"][i]
            final_df.drop(i, inplace=True)

    for i, row in final_df.iterrows():
        if final_df['RNAfold_structure'][i][18:28] == "..........":
            #final_df["motif_name"][i] = 'misfold_' + final_df["motif_name"][i]
            final_df.drop(i, inplace=True)

    for i, row in final_df.iterrows():
        if "(......)" in final_df['RNAfold_structure'][i][18:28]:
            #final_df["motif_name"][i] = 'misfold_' + final_df["motif_name"][i]
            final_df.drop(i, inplace=True)
    for i, row in final_df.iterrows():
        if "(.....)" in final_df['RNAfold_structure'][i][18:28]:
    ...
    final_df.to_csv("7_alt_mismatches_RNAfold_20200924.csv", index=False)
