...
def main():
    ...
    df_motifs = pd.read_csv("../motifs/core_kts.csv")
...
#Below this comment begins trying to match chip_structure against RNAfold's predicted secondary structure for a sequence.
    ...
    for i, row in final_df.iterrows():
        ...
        if final_df['chip_structure'][i][:10] != final_df['RNAfold_structure'][i][:10]:
            final_df.drop(i, inplace=True)
        elif ")" in final_df['chip_structure'][i][:18]:
            final_df.drop(i, inplace=True)
        else:
            pass
    print(final_df.head())
    ...
    final_df.to_csv("7iii_980_980.9_RNAfolded_20200922.csv", index=False)
